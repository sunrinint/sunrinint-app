...
const SchoolCard = ({
  ...
  return (
    <Wrapper>
      ...
      >
        <InfoContainer>
          <Typography.Body $color={colors.gray30}>
            선린인터넷고등학교 모바일 학생증
          </Typography.Body>
          <Column $gap={8}>
            <Typography.Name $color={colors.gray20}>{name}</Typography.Name>
            <Typography.SemiLabel $color={colors.gray60}>
              {info.grade}학년 {info.class}반 {info.number}번
            </Typography.SemiLabel>
            <Typography.Body $color={colors.gray30}>{email}</Typography.Body>
          </Column>
          <Typography.Body $color={colors.gray30}>
        ...
        <BarcodeContainer>
          ...
          <Typography.Body $color={colors.gray30}>{code}</Typography.Body>
        </BarcodeContainer>
      </Container>
    </Wrapper>
  );
